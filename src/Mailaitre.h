...
class Mailaitre {
private:
    std::string api_base = "https://mail.aitre.cc/api";
    ...
    http_request create_request(const std::string& endpoint, const std::string& method, const std::string& data = "") {
        ...
        request.headers().add(U("Cookie"), U("i18n_redirected=en-US"));
        request.headers().add(U("Host"), U("mail.aitre.cc"));
...
public:
    ...
    pplx::task<json::value> generate_email() {
        return make_api_call("/tempmail/generate", "POST","{}");
    }

    pplx::task<json::value> get_mails(const std::string& email) {
        return make_api_call("/tempmail/emails?email=" + utility::conversions::to_utf8string(web::uri::encode_data_string(utility::conversions::to_string_t(email))), "GET");
    }
};
